...
speciesIds <- c()
for(n in rownames(RA_corr)){
  id <- species_csv[species_csv$names==n, "ncbi_ids"]
  speciesIds <- c(speciesIds, id)
}

rownames(RA_corr) <- speciesIds
rownames(RD_corr) <- speciesIds
rownames(RL_corr) <- speciesIds
colnames(RA_corr) <- speciesIds
colnames(RD_corr) <- speciesIds
colnames(RL_corr) <- speciesIds
...
RA_phylum_results <- get_distributions(RA_corr, "phylum_id")
RD_phylum_results <- get_distributions(RD_corr, "phylum_id")
RL_phylum_results <- get_distributions(RL_corr, "phylum_id")

RA_phylum_plot <- boxplot_distances(RA_phylum_results, mean(RA_corr), "Phylum", "RA")
RD_phylum_plot <- boxplot_distances(RD_phylum_results, mean(RD_corr), "Phylum", "RD")
RL_phylum_plot <- boxplot_distances(RL_phylum_results, mean(RL_corr), "Phylum", "RL")
...
RA_class_results <- get_distributions(RA_corr, "class_id")
RD_class_results <- get_distributions(RD_corr, "class_id")
RL_class_results <- get_distributions(RL_corr, "class_id")

RA_class_plot <- boxplot_distances(RA_class_results, mean(RA_corr), "Class", "RA")
RD_class_plot <- boxplot_distances(RD_class_results, mean(RD_corr), "Class", "RD")
RL_class_plot <- boxplot_distances(RL_class_results, mean(RL_corr), "Class", "RL")

RA_order_results <- get_distributions(RA_corr, "order_id")
RD_order_results <- get_distributions(RD_corr, "order_id")
RL_order_results <- get_distributions(RL_corr, "order_id")

RA_order_plot <- boxplot_distances(RA_order_results, mean(RA_corr), "Order", "RA", include_not_in_dict=TRUE)
RD_order_plot <- boxplot_distances(RD_order_results, mean(RD_corr), "Order", "RD", include_not_in_dict=TRUE)
RL_order_plot <- boxplot_distances(RL_order_results, mean(RL_corr), "Order", "RL", include_not_in_dict=TRUE)
...
RA_mean <- mean(all_results[all_results$site=="RA", "dist"])
RD_mean <- mean(all_results[all_results$site=="RD", "dist"])
RL_mean <- mean(all_results[all_results$site=="RL", "dist"])
...
grid.arrange(
  top="Correlation of species within same taxonomical groups and between different taxonomical groups",
  p_RA, p_RD, p_RL,
  ncol=1
)
